...
public class AlertService : DiscordClientService
{
    ...
    private async void UserAlerts()
    {
        ...
        foreach (var item in userAlertsWithQuestions)
        {
            ...
            {
                if (question.IsSmallSwing()) 
                {
                    smallSwing.Add(item);
                }
                else if (question.IsBigSwing())
                {
                    bigSwing.Add(item);
                }
            }
    ...
    private async Task SendSmallSwingMessageAsync(Tuple<UserQuestionAlert, AlertQuestion> tup)
    {
        var (alert, question) = tup;
        var target = await Client.GetUserAsync(alert.UserId);
        string message = $"{question.Title} has been resolved. The answer is {question}"; //todo make text
        await target.SendMessageAsync(message);
    }
...
/// <returns>IEnumerable of pairs of Alerts and Questions </returns>
    private IEnumerable<Tuple<UserQuestionAlert,AlertQuestion>> JoinWithQuestions(IEnumerable<UserQuestionAlert> userAlerts)
    {
        ...
        return userQuestionAlerts.Join(filtered, x => x.QuestionId, y => y.Result!.Id, (x, y) => new Tuple<UserQuestionAlert, AlertQuestion>(x, y.Result!));
    }
